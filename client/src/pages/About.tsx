  ...
  return (
    <SiteShell>
      ...
      <section className="team-section">
        <div className="team-heading"><PageEyebrow>Meet the chairs</PageEyebrow><h2>Specialists with a <em>specific point of view.</em></h2></div>
        <div className="team-grid">
          {stylists.map((stylist) => {
            const saved = isSavedStylist(stylist.id);
            return (
              <article className="team-card" key={stylist.id}>
                <div className="team-card-top">
                  <div className="team-avatar" style={{ backgroundColor: stylist.accent }}>{stylist.initials}</div>
                  <div className="flex gap-1.5">
                    <button className={saved ? "icon-btn saved" : "icon-btn"} aria-label={saved ? `Unsave ${stylist.name}` : `Save ${stylist.name}`} onClick={() => toggleSavedStylist(stylist.id, stylist.name)}>
      ...
      <section className="contact-section">
        <div><PageEyebrow>Come by</PageEyebrow><h2>Find a little <em>time for you.</em></h2><p>{ADDRESS}<br />Tuesday–Friday 9–6 · Saturday–Sunday 10–5</p>
