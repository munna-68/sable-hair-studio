    ...
    <SiteShell>
      ...
      <section className="booking-layout">
        <div className="booking-steps">
          ...
          <section className={service ? "booking-step" : "booking-step muted-step"}><div className="step-title"><span>02</span><div><h2>Are you new to Sable?</h2><p>We ask so chemical services begin safely and with context.</p></div></div><div className="choice-pair"><button className={isNewClient ? "choice-card selected" : "choice-card"} onClick={() => { setIsNewClient(true); toast.info("First visit — welcome.", { description: "Chemical services will start with a consult." }); }}><UserRound size={18} /><b>First visit</b><small>I am new to Sable</small></button><button className={!isNewClient ? "choice-card selected" : "choice-card"} onClick={() => { setIsNewClient(false); toast.success("Welcome back.", { description: "Full booking unlocked for chemical services." }); }}><Check size={18} /><b>Returning client</b><small>I have a service history</small></button></div>{consultationRequired && <div className="consultation-notice"><ShieldCheck size={19} /><p><b>Consultation required before confirmation.</b> Because this is your first chemical-service visit, we will start with a 15-minute consultation or patch-test step. Your full service is not confirmed until it is complete.</p></div>}</section>
